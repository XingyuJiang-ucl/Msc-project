...
output_base  = r'D:\Msc project\code\2D Slices 224 Tumor 1HU'
...
def window_hu(slice_2d: np.ndarray, hu_min: float, hu_max: float) -> np.ndarray:
    return np.clip(slice_2d, hu_min, hu_max)
...
for i in range(589):
    ...
    case_out_dir  = os.path.join(split_dir, case_name)
    os.makedirs(case_out_dir, exist_ok=True)

    if not os.path.exists(img_path) or not os.path.exists(seg_path):
        print(f"[{split}] Missing {case_name}, skipping")
        continue
    ...
    for z in range(num_slices):
        ...
        # apply HU window
        slice_2d = vol_rs[z, :, :]
        win      = window_hu(slice_2d, hu_min, hu_max)
        ...
        slice_u8 = (norm * 255).astype(np.uint8)

        img_pil = Image.fromarray(slice_u8)  # mode 'L'
        img_resized = img_pil.resize(TARGET_SIZE, resample=Image.BICUBIC)

        out_fname = os.path.join(case_out_dir, f'slice_{saved:03d}.png')
